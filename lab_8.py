...
print(f"Source mean: {float((ds_fine['precip'].mean())):.3} ")
print(f"Target mean: {float((ds_bil['precip'].mean())):.3}")
print(f"Source min: {float((ds_fine['precip'].min())):.3}")
print(f"Target min: {float((ds_bil['precip'].min())):.3}")
print(f"Source max: {float((ds_fine['precip'].max())):.3}")
print(f"Target max: {float((ds_bil['precip'].max())):.3}")
print(f"Source NaN count: {int(ds_fine['precip'].isnull().sum())}")
print(f"Target NaN count: {int(ds_bil['precip'].isnull().sum())}")
...
ax.plot(lats_f, f_src, color='#FA003F', label='Fine (0.25°)')      # fine grid line
ax.step(lats_c, f_tgt, color='#FFCE1B', label='Conservative (1°)') # coarse step plot
